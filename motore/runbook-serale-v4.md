...
4. Scarica i dati: `data/log.txt`, `data/strumenti.csv`, `data/prezzi.csv` (stessa forma di URL con nocache). Freschezza: nel log la riga "ultima data: AAAA-MM-GG" deve essere la seduta di oggi (o l'ultima seduta di Borsa se oggi è festivo). Se i dati sono vecchi, riprova ogni 5 minuti (`sleep 300`) fino a 5 volte (25 minuti in tutto: il download di riserva dal Mac mini parte alle 19:35), poi sezione Errori.

## 2. Canale

`python3 segnali_v4.py leggi_canale --stato stato.json` legge i comandi scritti da Ugo nel canale nelle ultime 24 ore. L'output contiene anche `diagnostica` (aggiornamenti ricevuti e scartati, per esempio messaggi in una chat diversa dal canale) e `tg_last_update_id`: riportali nel diario se non c'è nessun comando ma Ugo doveva averne scritto uno (ESEGUITO qta prezzo, NON ESEGUITO, VENDUTO qta prezzo, NON DISPONIBILE ticker, SALDO euro, PAUSA, RIPRENDI) e li applica allo stato. Gli esiti restano in `stato.json` (campo `eventi_pendenti`) e il motore li mette da solo nel messaggio serale. Leggi l'output: se un comando è "non capito", non interpretarlo tu: verrà segnalato nel messaggio.
